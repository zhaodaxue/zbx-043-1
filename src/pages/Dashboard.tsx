...
export default function Dashboard() {
  const { stations, selectedStation, loading, error, loadData, setSelectedStation } = useDashboardStore();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-6">
      <div className="max-w-[1600px] mx-auto">
        <DashboardHeader totalStations={stations.length} highRiskCount={highRiskCount} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6">
          <div className="lg:col-span-7 bg-slate-800/50 backdrop-blur-sm rounded-2xl border border-slate-700/50 p-5 shadow-xl">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-semibold text-slate-100">各站点净客流排名</h2>
              <span className="text-xs text-slate-500 ml-2">点击站点查看时段走势</span>
            </div>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                {selectedStationData && (
                  <div className="text-sm">
                    ...
                    <span className="text-cyan-400 font-medium">{selectedStationData.stationName}</span>
                  </div>
                )}
              </div>
              <div className="h-[240px]">
                <LineChart station={selectedStationData} />
              </div>
              {selectedStationData && (
                <div className="grid grid-cols-3 gap-3 mt-4 pt-4 border-t border-slate-700/50">
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-xs text-slate-500 mt-1">净客流</div>
                  </div>
                </div>
              )}
            </div>
